...
export function ChatWidget({ businessId, businessName, businessAvatar, isOpen, onOpenChange }: ChatWidgetProps) {
  ...
  return (
    <>
      ...
      {widgetState === 'open' && (
        <div className="fixed inset-0 sm:inset-auto sm:bottom-6 sm:right-6 z-50 sm:w-[380px] sm:h-[520px] sm:rounded-2xl bg-card sm:border border-border shadow-2xl flex flex-col overflow-hidden animate-in slide-in-from-bottom-4 fade-in duration-200">
          {/* Header */}
          <div className="flex items-center justify-between px-4 py-3 bg-primary text-primary-foreground shrink-0">
            <div className="flex items-center gap-2.5">
              {businessAvatar ? (
                <img src={businessAvatar} alt="" className="w-9 h-9 rounded-full object-cover border-2 border-primary-foreground/30" />
              ) : (
                  ...
                  {!isMe && (
                    <div className="flex items-center gap-1.5 mb-0.5 ml-1">
                      {m.senderPhoto ? (
                        ...
                        <img src={businessAvatar} alt="" className="w-4 h-4 rounded-full object-cover" />
                      ) : (
                        <div className="w-4 h-4 rounded-full bg-primary/10 flex items-center justify-center text-primary text-[8px] font-bold">
                          {(m.senderName || businessName).charAt(0)}
                        </div>
                      )}
                      <span className="text-[10px] text-muted-foreground font-medium">{m.senderName || businessName}</span>
                    </div>
                  )}
                  <div className={`max-w-[80%] px-3 py-2 text-[13px] leading-snug ${
                    isMe
