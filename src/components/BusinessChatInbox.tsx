...
export function BusinessChatInbox({ businessId }: BusinessChatInboxProps) {
  ...
  // 1. Ensure ownerId is set on the business doc + backfill chat participants
  useEffect(() => {
    ...
    const backfill = async () => {
      try {
        ...
        // Also backfill participants on existing chats for this business
  ...
  // 2. Real-time thread list — all chats where this user is a participant
  useEffect(() => {
    if (!open || !user) return;
  ...
  // 3. Real-time messages for selected thread
  useEffect(() => {
  ...
  // 5. Focus input when thread changes
  useEffect(() => {
    if (selectedThread) {
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    setDraft('');
    await sendMessage(selectedThread.id, user.id, 'business', text);
  };
  ...
  return (
    <div className="fixed bottom-6 left-6 z-50 w-[700px] h-[520px] rounded-2xl bg-white border border-border shadow-2xl flex overflow-hidden animate-in slide-in-from-bottom-4 fade-in duration-200">
          ...
          <>
            ...
            <div className="px-4 py-3 border-b border-border flex items-center justify-between bg-white">
              <div className="flex items-center min-w-0">
                ...
                <div className="w-9 h-9 bg-primary/10 rounded-xl flex items-center justify-center text-primary font-bold text-sm flex-shrink-0">
              ...
              {messages.map((m) => {
                ...
                return (
                  <div key={m.id} className={`flex flex-col ${isMe ? 'items-end' : 'items-start'}`}>
                    <div className={`max-w-[75%] px-4 py-2.5 rounded-2xl text-sm leading-snug shadow-sm ${
                      isMe
                    ...
                    {displayTime && (
                      <span className="text-[10px] text-muted-foreground mt-1 px-1">{displayTime}</span>
                    )}
                  </div>
                );
              })}
