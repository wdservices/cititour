    ...
    <div className="space-y-6 relative">
                  ...
                  <tr key={user.id} className="border-b border-sand-100 last:border-b-0">
                    ...
                    <td className="py-3 text-right">
                      <button
                        ...
                        className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold text-coral hover:bg-coral/10 border border-coral/20"
      ...
      {(confirmStep === 1 || confirmStep === 2) && targetUser && (
        <div className="fixed inset-0 z-50 bg-ink/50 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="w-full max-w-lg bg-white rounded-2xl border border-sand-200 shadow-2xl overflow-hidden">
            <div className="flex items-start justify-between p-5 border-b border-sand-200">
              ...
              <button
                onClick={closeDelete}
                disabled={busy}
                className="p-1.5 rounded-lg hover:bg-ink/5 text-ink/50 hover:text-ink disabled:opacity-40"
              >
                <X className="h-5 w-5" />
              </button>
            </div>
              ...
              {confirmStep === 1 ? (
                <>
                  <div className="text-sm text-ink/70 space-y-2">
                    <p>
                      <strong className="text-coral">This cannot be undone directly.</strong> The system will:
                    </p>
                    <ul className="list-disc pl-5 space-y-1">
                      <li>Create a full archive snapshot in <code className="px-1.5 py-0.5 rounded bg-ink/5 text-xs">deleted_accounts_log</code></li>
                      <li>Hard-delete <strong>users, wallet, businesses, marketplace, house_listings</strong> owned by this user</li>
                      <li>Destroy all associated Cloudinary images (logos, photos, room images)</li>
                    </ul>
                    <p className="text-ink/60 text-xs mt-2">
                      Tip: if deleted by mistake, the archive snapshot can be restored via the <code className="px-1.5 py-0.5 rounded bg-ink/5 text-xs">restoreDeletedAccount</code> callable using the archive id logged in the success toast.
                    </p>
                  </div>
                  <div className="flex items-center justify-end gap-2 pt-2">
                    <button
                      onClick={closeDelete}
                      disabled={busy}
                      className="px-4 py-2 rounded-xl text-sm font-semibold text-ink/70 hover:bg-ink/5 disabled:opacity-40"
                    >
                      Cancel
                    </button>
                    <button
                      onClick={proceedToStep2}
                      className="px-4 py-2 rounded-xl text-sm font-semibold bg-coral text-white hover:bg-coral/90"
                    >
                ...
                <>
                  <div>
                    <label className="text-xs font-semibold uppercase tracking-wider text-ink/60 block mb-2">
                    ...
                    <input
                      ...
                      className="w-full px-3 py-2 rounded-xl border border-sand-200 bg-white text-sm focus:outline-none focus:ring-2 focus:ring-coral/30"
                    />
                    <p className="text-xs text-ink/50 mt-2">
                      Type <span className="font-mono text-coral font-semibold">{targetUser.email}</span> to enable the delete button.
                    </p>
                  </div>
                  <div className="flex items-center justify-end gap-2 pt-2">
                    <button
                    ...
                    <button
                      ...
                      className="px-4 py-2 rounded-xl text-sm font-semibold bg-coral text-white hover:bg-coral/90 disabled:opacity-40 inline-flex items-center gap-2"
                    >
